...
| F13 | L | `SessionDiscovery.swift:179` | `mcpRegistrationCutoff()` reads `~/.claude.json` mtime synchronously. Same call site (`listSessions()`) already runs off-main per F1/F4, so this is fine. | No change. |
| F14 | L | `MainWindowController.swift:131` | Toolbar-action `copyTranscriptReport` runs `NSAlert.runModal()` on main — intentional confirmation UI; not a freeze. | No change. |
